...
local function prepForDiscard(orderCard)
    ...
    -- This is'nt null, right?
    if orderCard == nil then
        print("Doug: adding nil to ordersToDiscard")
    end
    table.insert(ordersToDiscardGUIDs, orderCard.getGUID())
end
...
local function discardCard(card, opt_callback)
    print("Doug: discardCard 001 orderDiscardTile = ", orderDiscardTile)
    print("Doug: discardCard 002 card = ", card)
    ...
    print("Doug: discardCard 003 card = ", card)
    Wait.time(function()
        print("Doug: discardCard 004 card = ", card)
        if opt_callback then
            print("Doug: discardCard 005 card = ", card)
            opt_callback()
        end
    end, waitForPlacementSec)
end
...
local function isCollection(object)
...
local function safeGetNextCardFromOrderDeck(callback)
    ...
    if orderDeck == nil then
        ...
        local retVal
        if isCollection(orderDeck) then
            retVal = orderDeck.takeObject()
        else
            retVal = orderDeck
        end
        callback(retVal)
...
local function discardNextOrderCard()
    print("Doug: discardNextOrderCard")
    print("Doug: ordersToDiscardGUIDs", ordersToDiscardGUIDs)

    if #ordersToDiscardGUIDs == 0 then
        print("Doug: discardNextOrderCard returning")
        return
    end

    local cardGUID = table.remove(ordersToDiscardGUIDs, 1)

    local card = getObjectFromGUID(cardGUID)
    -- It's possible the card got cleaned up manually already: gracefully fail/continue.
    if not card then
