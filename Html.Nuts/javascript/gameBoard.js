define([
	...
	'javascript/rowTypes',
	'dojo/domReady!'
], function(dom, query, domStyle, domClass, beltUtils, cards, conveyorTiles, debugLog, gameUtils, markers, rowTypes){
	var rowZUIndex = 0
	...
	function addRowToStripAndReturnRowContent(parentNode, rowIndex, rowType, opt_configs) {
	...
	function addEntityNameAndIndex(parentNode, entityName, entityIndex) {
		var entityNameNode = gameUtils.addDiv(parentNode, ["entityTitle"], "entityTitle", entityName)
		var entityIndexNode = gameUtils.addDiv(parentNode, ["entityIndex"], "entityIndex", entityIndex)
	...
	function getNumColumnsThisPage(totalColumnCount, maxColumnsPerPage, numColumnsAlreadyHandled) {
		var numColumnsLeft = totalColumnCount - numColumnsAlreadyHandled
		var retVal = Math.min(numColumnsLeft, maxColumnsPerPage)
		debugLog.debugLog("GameBoard", "Doug: getNumColumnsThisStrip retVal = " + retVal)
		return retVal
	}

	function addStandardSlotWithElementAndBelt(parentNode, rowIndex, columnIndex, opt_configs) {
		// Column index is 0-based.
		debugLog.debugLog("GameBoard", "Doug: addStandardSlotWithElementAndBelt: rowIndex = " + rowIndex + " columnIndex = " + columnIndex)
		var configs = opt_configs ? opt_configs : {}

		var classArray = configs.classArray
		var tweakElement = configs.tweakElement
		var skipElement = configs.skipElement
		var entityName = configs.entityName
		var entityIndex = configs.entityIndex
	...
	function addNColumnRowWithElements(parentNode, rowIndex, rowType, numColumnsThisPage, numColumnsAlreadyHandled, opt_configs) {
		debugLog.debugLog("GameBoard", "Doug: addNColumnRowWithElements: rowIndex = " + rowIndex)
		debugLog.debugLog("GameBoard", "Doug: addNColumnRowWithElements: numColumnsThisPage = " + numColumnsThisPage)
		debugLog.debugLog("GameBoard", "Doug: addNColumnRowWithElements: numColumnsAlreadyHandled = " + numColumnsAlreadyHandled)

		var configs = opt_configs ? opt_configs : {}

		var entityName = rowTypes.getRowEntityName(rowType)
		var content = addRowToStripAndReturnRowContent(parentNode, rowIndex, rowType, configs)
		...
		for (let i = 0; i < numColumnsThisPage; i++) {
			...
			addStandardSlotWithElementAndBelt(content, rowIndex, numColumnsAlreadyHandled + i, elementConfigs)
		}

		return content
	}

	function addNColumnRowWithNumbers(parentNode, rowIndex, rowType, numColumnsThisPage, numColumnsAlreadyHandled, opt_configs) {
		debugLog.debugLog("GameBoard", "Doug: addNColumnRowWithNumbers: numColumnsAlreadyHandled = " + numColumnsAlreadyHandled)
		var content = addRowToStripAndReturnRowContent(parentNode, rowIndex, rowType, opt_configs)
	...
	function addNColumnRowWithConveyors(parentNode, rowIndex, rowType, numColumnsThisPage, numColumnsAlreadyHandled, opt_configs) {
		var content = addRowToStripAndReturnRowContent(parentNode, rowIndex, rowType, opt_configs)

		var configs = opt_configs ? opt_configs : {}
		configs.skipElement = true
		for (let i = 0; i < numColumnsThisPage; i++) {
			addStandardSlotWithElementAndBelt(content, rowIndex, numColumnsAlreadyHandled + i, configs)
		}
		return content
	}

	function tweakBoxesRowCardSlot(node)	{
		var cardSlotHeight = rowTypes.standardRowHeight/2 - 2 * (gameUtils.boxesRowMarginTop)
		domStyle.set(node, {
	...
	function addPagesWithSidebar(bodyNode, startingPageNode, orderedRowTypes, totalNumColumns, maxRowsPerPage) {
		var pageNodes = []
	...
	function addPagesWithNextNColumns(bodyNode, orderedRowTypes, maxRowsPerPage, totalNumColumns, maxColumnsPerPage, numColumnsAlreadyHandled) {
		var allPageNodes = []
		var numColumnsThisStrip = getNumColumnsThisPage(totalNumColumns, maxColumnsPerPage, numColumnsAlreadyHandled)
		...
		for (let i = 0; i < orderedRowTypes.length; i++) {
			...
			switch (rowType) {
				case rowTypes.RowTypes.Number:
					addNColumnRowWithNumbers(currentPageNode, rowIndex, rowType, numColumnsThisStrip, numColumnsAlreadyHandled, {
						classArray: ["numbers"],
					})
					break
				case rowTypes.RowTypes.Dispenser:
					addNColumnRowWithElements(currentPageNode, rowIndex, rowType, numColumnsThisStrip, numColumnsAlreadyHandled, {
				...
				case rowTypes.RowTypes.Path:
					addNColumnRowWithConveyors(currentPageNode, rowIndex, rowType, numColumnsThisStrip, numColumnsAlreadyHandled, {
		...
		debugLog.debugLog("GameBoard", "Doug: addPagesWithNextNColumns: at the end numColumnsThisPage = " + numColumnsThisStrip)
		return {
	...
	function addGameBoard(configs) {
		...
		if (pageless || maxColumnsPerPage >= totalNumColumns) {
			...
			if (thisBoardHasSidebar) {
				var pageNodes = addPagesWithSidebar(bodyNode, orderedRowTypes, totalNumColumns, maxRowsPerPage)
				console.assert(pageNodes.length == 1)
				pageNode = pageNodes[0]
			}

			var retVal = addPagesWithNextNColumns(bodyNode,
				orderedRowTypes, maxRowsPerPage,
				totalNumColumns, totalNumColumns,
				0, pageNode)
			console.assert(retVal, "Doug: addGameBoard: retVal is null")
			console.assert(retVal.allPageNodes, "Doug: addGameBoard: retVal.allPageNodes is null")
			var allPageNodes = retVal.allPageNodes
			console.assert(allPageNodes.length == 1, "Doug: addGameBoard: allPageNodes.length = " + allPageNodes.length)
			...
			if (thisBoardHasSidebar) {
				addPagesWithSidebar(bodyNode, orderedRowTypes, totalNumColumns, maxRowsPerPage)
			}

			var numColumnsAlreadyHandled = 0
			while (numColumnsAlreadyHandled < totalNumColumns) {
				debugLog.debugLog("GameBoard", "Doug: totalNumColumns = " + totalNumColumns)
				debugLog.debugLog("GameBoard", "Doug: numColumnsAlreadyHandled = " + numColumnsAlreadyHandled)
				var retVal = addPagesWithNextNColumns(bodyNode,
					orderedRowTypes, maxRowsPerPage,
					totalNumColumns, maxColumnsPerPage,
					numColumnsAlreadyHandled)
				console.assert(retVal, "Doug: addGameBoard: retVal is null")
				console.assert(retVal.numColumnsThisStrip, "Doug: addGameBoard: retVal.numColumnsThisStrip is null")
				...
				debugLog.debugLog("GameBoard", "Doug: final numColumnsAlreadyHandled = " + numColumnsAlreadyHandled)
			}
		}
	}
	...
	function storeConveyorTileId(rowIndex, columnIndex, conveyorTileId) {
		...
		conveyorTileIdsByRowThenColumn[rowIndexString][columnIndexString] = conveyorTileId
	}
	...
	function placeConveyorTileOnBoard(rowIndex, columnIndex, opt_classArray) {
		...
		var conveyorTile = conveyorTiles.addCrossTile(slot, conveyorTileId, opt_classArray)
	...
	function highlightConveyorTile(rowIndex, columnIndex, color, opt_translucentColor) {
		var conveyorTile = getStoredConveyorTile(rowIndex, columnIndex)
		if (conveyorTile) {
