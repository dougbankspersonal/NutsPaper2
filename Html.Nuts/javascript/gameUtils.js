define([
	...
	"dojo/query",
	'dojo/domReady!'
], function(dom, debugLog, rowTypes, versionDetails, domConstruct, domStyle, query) {
	...
	var conveyorTileHeight = rowTypes.standardRowHeight - 2 * conveyorTileOnBoardTopMargin
	...
	var beltCenterOffsetInConveyorTile = slotWidth/2 - conveyorTileOnBoardLeftMargin - conveyorTileBorder
	...
	{
		debugLog.debugLog("ScoringTrack", "extendOptClassArray: opt_classArray == " + opt_classArray)
		debugLog.debugLog("ScoringTrack", "extendOptClassArray: newClassOrClasses == " + newClassOrClasses)
		var classArray = opt_classArray ? opt_classArray: []
		console.assert(typeof classArray === 'object', "classArray is not an object")
	...
	function getPageWidth(configs) {
		if (configs.demoBoard) {
			var demoBoardWidth = sidebarWidth + versionDetails.getTotalNumColumns() * slotWidth + 2 * standardBorderWidth
		...
		if (configs.demoBoard) {
			...
			if (lastRowType == rowTypes.RowTypes.Boxes) {
				...
				if (configs.noSpaceForCards) {
					return 2 * standardBorderWidth + numNonOrderRows * rowTypes.standardRowHeight + rowTypes.boxesRowHeight
				}
				else
				{
					return 2 * standardBorderWidth + numNonOrderRows * rowTypes.standardRowHeight + boxesRowMarginTop + cardBorderWidth + cardHeight + cardBorderWidth
				}
			} else {
