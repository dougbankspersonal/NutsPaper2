define([
	...
	function getCurvePoints() {
		...
		var endX = gameUtils.conveyorTileInnerWidth - gameUtils.beltCenterOffsetInConveyorTile
	...
	function addEmptyConveyorTileElement(parentNode, conveyorTileId, opt_classArray) {
		var classArray = gameUtils.extendOptClassArray(opt_classArray, "conveyor_tile")
	...
	function addStraightBelt(conveyorTile, isLeft) {
		var belt = gameUtils.addDiv(conveyorTile, ["belt", isLeft ? "left" : "right"], isLeft ? "leftBelt" : "rightBelt")
		var xOffset = isLeft ? gameUtils.beltCenterOffsetInConveyorTile : gameUtils.conveyorTileInnerWidth - gameUtils.beltCenterOffsetInConveyorTile
	...
	function addRightToLeftBelt(conveyorTile) {
		...
		for (let index = 0; index < curvePoints.length; index++) {
			...
			beltUtils.addBeltSegment(belt, gameUtils.conveyorTileInnerWidth - curvePoint.xOffset, curvePoint.yOffset, -curvePoint.rads)
		}
		return belt
	}
	...
	function addSplitterJoinerTile(parentNode, conveyorTileId, arity, opt_classArray) {
		debugLog.debugLog("ConveyorTiles", "Doug: addSplitterJoinerTile arity == " + arity)
		var conveyorTile = addEmptyConveyorTileElement(parentNode, conveyorTileId, opt_classArray)
	...
	function addConveyorTilesPage(bodyNode, numConveyorTiles, opt_isSplitterJoiner) {
	...
	function createConveyorTiles(totalNumTiles, opt_isSplitterJoiner) {
		...
		for (i = 0; i < numPages; i++) {
			var numStripsThisPage = Math.min(stripsPerPage, totalNumTiles - i * stripsPerPage)
